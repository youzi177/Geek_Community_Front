...
layui.define(
  ['layer', 'laytpl', 'form', 'element', 'upload', 'util'],
  function (exports) {
    var $ = layui.jquery,
      layer = layui.layer,
      laytpl = layui.laytpl,
    ...
    if (device.ie && device.ie < 8) {
      layer.alert('如果您非得使用 IE 浏览器访问geek极客社区，那么请使用 IE8+');
    }
        ...
        var log = {},
          mod = {
            face: function (editor, self) {
              ...
              for (var key in face) {
                str +=
                  '<li title="' + key + '"><img src="' + face[key] + '"></li>';
              }
              str =
                '<ul id="LAY-editface" class="layui-clear">' + str + '</ul>';
              layer.tips(str, self, {
                tips: 3,
                time: 0,
            ...
            yulan: function (editor) {
              ...
              layer.open({
                ...
                scrollbar: false,
                content:
                  '<div class="detail-body" style="margin:20px;">' +
                  content +
                  '</div>',
              });
            },
      ...
      content: function (content) {
        ...
        content = fly
          ...
          .replace(/a\([\s\S]+?\)\[[\s\S]*?\]/g, function (str) {
            ...
            if (!href) return str;
            var rel =
              /^(http(s)*:\/\/)\b(?!(\w+\.)*(sentsin.com|layui.com))\b/.test(
                href.replace(/\s/g, ''),
              );
            return (
              '<a href="' +
          ...
          fly.json(
            ...
            function (res) {
              if (res.status === 0 && res.count > 0) {
                var msg = $(
                  '<a class="fly-nav-msg" href="javascript:;">' +
                    res.count +
                    '</a>',
                );
                elemUser.append(msg);
    ...
    */
    ...
    fly.activate = function (email) {
      fly.json('/api/activate/', {}, function (res) {
        if (res.status === 0) {
          layer.alert(
            '已成功将激活链接发送到了您的邮箱，接受可能会稍有延迟，请注意查收。',
            {
              icon: 1,
            },
          );
